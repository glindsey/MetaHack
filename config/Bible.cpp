...
namespace Config
{
  ...
  json& Bible::categoryData(std::string name)
  {
  ...
  void Bible::loadCategoryIfNecessary(std::string name)
  {
  ...
  void Bible::loadTemplateComponentsFor(std::string name)
  {
    StringPair namePair = StringTransforms::splitName(name);
  ...
  Bible& bible()
  {
    static std::unique_ptr<Bible> ptr;
