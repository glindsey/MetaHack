...
void MapView::set_view(sf::RenderTarget & target, RealVec2 center, float zoom_level)
{
  ...
  unsigned int inventory_area_width = config.get("inventory_area_width");
  unsigned int status_area_height = config.get("status_area_height");
  unsigned int messagelog_area_height = config.get("messagelog_area_height");
