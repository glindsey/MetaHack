...
inline float calc_vis_distance(IntegerVec2 s1, IntegerVec2 s2)
{
  float x_dist = static_cast<float>(s1.x - s2.x);
  float y_dist = static_cast<float>(s1.y - s2.y);
...
inline RealVec2 to_v2f(IntegerVec2 vec)
{
...
inline bool adjacent(IntegerVec2 first, IntegerVec2 second)
{
