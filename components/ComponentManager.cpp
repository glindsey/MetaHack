...
namespace Components
{
    ...
    ComponentManager(gameState)
  {
    ...
    JSONUtils::doIfPresent(j, "noun",               [this](auto const& value) { noun            = value; });
    JSONUtils::doIfPresent(j, "adjective-material", [this](auto const& value) { adjMaterial     = value; });
  ...
  void ComponentManager::clone(EntityId original, EntityId newId)
  {
    ...
    noun           .cloneIfExists(original, newId);
    adjMaterial    .cloneIfExists(original, newId);
  ...
  void ComponentManager::populate(EntityId id, json const& j)
  {
    ...
    JSONUtils::doIfPresent(j, "noun",               [this, &id](auto const& value) {      properName.update(id, value); });
    JSONUtils::doIfPresent(j, "adjective-material", [this, &id](auto const& value) {      properName.update(id, value); });
  ...
  json ComponentManager::toJSON()
  {
    ...
    j["proper-name"]        = noun;
    j["adjective-material"] = adjMaterial;
  ...
  json ComponentManager::toJSON(EntityId id)
  {
    ...
    if (noun.existsFor(id))           j["noun"]               = noun.of(id);
    if (adjMaterial.existsFor(id))    j["adjective-material"] = adjMaterial.of(id);
